-- SangHub_BetaBF_custom.lua
-- Combined full script: GUI (Sang Hub) + Shop & Mics split layout + Player ESP (name) + Fruit ESP (source mapping)
-- Click sound id: 12221967, Toggle icon: 92088814301938
-- Paste this LocalScript into StarterPlayerScripts or PlayerGui
...
-- Get_Fruit mapping (copied from your source)
...
-- Player ESP storage and functions (name only)
local playerESPs = {} -- [player] = BillboardGui
local playerConn = nil

local function createPlayerNameESP(plr)
    if not plr or not plr.Character then return end
    local root = plr.Character:FindFirstChild("HumanoidRootPart") or plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.RootPart
...
-- Fruit ESP using your source: scan workspace and show name on Tools/Models with "Fruit" in name
...
local function createFruitESP(item)
    ...
    tl.TextSize = 14
    tl.TextColor3 = Color3.fromRGB(255,100,100) -- red for fruit
...
-- add small spacer
local spacer = new("Frame", {Parent = leftFrame, Size = UDim2.new(1,0,0,6), BackgroundTransparency = 1})
local spacer2 = new("Frame", {Parent = rightFrame, Size = UDim2.new(1,0,0,6), BackgroundTransparency = 1})

-- Click sound instance (placed under screenGui)
local clickSound = new("Sound", {Parent = screenGui, SoundId = CLICK_SOUND_ID, Volume = 1})

-- create toggles on left: Player ESP, Fruit ESP (Fruit uses provided source logic)
...
print("SangHub_BetaBF_custom loaded. Left: Player ESP + Fruit ESP. Right column empty (FRUIT title present).")
